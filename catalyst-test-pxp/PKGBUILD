...
depends=('linux>=3.0' 'linux<4.1' 'linux-headers' 'xorg-server>=1.7.0' 'xorg-server<1.17.0' 'libxrandr' 'libsm' 'fontconfig' 'libxcursor' 'libxi' 'gcc-libs' 'gcc>4.0.0' 'make' 'patch' 'libxinerama')
...
build() {
  ## Unpack archive
#       /bin/sh ./amd-catalyst-13.11-beta\ V9.4-linux-x86.x86_64.run  --extract archive_files
#       /bin/sh ./amd-catalyst-${pkgver}-betaV${_betano}-linux-x86.x86_64.run --extract archive_files
#       /bin/sh ./amd-catalyst-13.15.100.1-x86.x86_64.run --extract archive_files
