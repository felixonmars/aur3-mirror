...
url="http://love2d.org/"
license=('zlib')
depends=('lua' 'physfs' 'freetype2' 'devil' 'mpg123' 'openal' 'libvorbis' 'libmodplug' 'sdl' 'shared-mime-info' 'hicolor-icon-theme' 'desktop-file-utils')
makedepends=('wget')
...
build() {
  cd "$srcdir/love-HEAD"
  ./configure --prefix=/usr LDFLAGS="$LDFLAGS -Wl,--no-as-needed"
  make
}
