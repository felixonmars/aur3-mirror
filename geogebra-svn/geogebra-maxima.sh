...
# Run
if [ -z "$GGB_PATH" ]; then
	exec java $JAVA_OPTS -jar /usr/share/java/geogebra/geogebra.jar --showSplash=false --CAS=Maxima $GG_OPTS
else
	exec java $JAVA_OPTS -jar /usr/share/java/geogebra/geogebra.jar --showSplash=false --CAS=Maxima $GG_OPTS "$GGB_PATH"
fi
