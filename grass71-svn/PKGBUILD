...
pkgver=0
pkgrel=5
pkgdesc="GRASS GIS (devel 7.1.x SVN branch): geospatial data management and analysis, image \
...
source=("${pkgname}::svn+http://svn.osgeo.org/grass/grass/trunk")
md5sums=('SKIP')
...
prepare() {
  ...
  # sed -i "s,PATH=\"\$(ARCH_DISTDIR)/bin:\$(GISBASE)/bin:\$\$PATH\",PATH=\"\$(ARCH_DISTDIR)/bin:\$(GISBASE)/bin:${srcdir}/${pkgname}:\$\$PATH\"," include/Make/Rules.make
  ...
  # Custom desktop file:
  sed -i -e "s,^Name=GRASS GIS 7\$,Name=GRASS GIS 7.1.x SVN r${pkgver}," \
...
package() {
  ...
  sed -i "s,${pkgdir},,g" "${pkgdir}/opt/${pkgname}/include/Make/Platform.make" \
                          ...
                          "${pkgdir}/opt/${pkgname}/bin/grass71" \
                          "${pkgdir}/opt/${pkgname}/demolocation/.grassrc71"

  # Link GRASS exec script in /usr/bin under a custom name.
  # This allows e.g. grass70 and grass70-svn be co-installed:
  mkdir -p "${pkgdir}/usr/bin"
  ln -sf "/opt/${pkgname}/bin/grass71" "${pkgdir}/usr/bin/grass71-svn"
  ...
  sed -i '1 s/python/python2/' "${pkgdir}/opt/${pkgname}/bin/grass71"
