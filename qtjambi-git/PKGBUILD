...
build() {
	...
	ant all qtjambi.source tools.juic.make
}
...
package() {
	...
	install -m644 build/platform-output/lib/libcom_trolltech_tools_designer.so.1 "$pkgdir/usr/lib/jni/"
	
	# Install juic
	install -Dm755 build/qmake-juic/juic "$pkgdir/usr/bin/juic"
}