# Maintainer: Ner0

pkgname=cinnamon-themes
pkgver=1.0
pkgrel=11
pkgdesc="Metapackage for Cinnamon Themes"
arch=('any')
url="http://cinnamon-spices.linuxmint.com/themes"
license=('GPL')
depends=('cinnamon-theme-ambiance'
         'cinnamon-theme-baldr'
         'cinnamon-theme-bluebird'
         'cinnamon-theme-cinnamint'
         'cinnamon-theme-delorean-dark'
         'cinnamon-theme-eleganse'
         'cinnamon-theme-elementary-luna'
         'cinnamon-theme-facebook'
         'cinnamon-theme-faience'
         'cinnamon-theme-glass'
         'cinnamon-theme-gnome'
         'cinnamon-theme-google+'
         'cinnamon-theme-grayscale'
         'cinnamon-theme-helios'
         'cinnamon-theme-holo'
         'cinnamon-theme-ics'
#        'cinnamon-theme-jelly-bean'
         'cinnamon-theme-lambda'
         'cinnamon-theme-loki'
         'cinnamon-theme-midnight'
         'cinnamon-theme-minty'
         'cinnamon-theme-minty-arch'
         'cinnamon-theme-nightfall'
         'cinnamon-theme-nightlife'
         'cinnamon-theme-odin'
         'cinnamon-theme-sunset'
         'cinnamon-theme-tout'
         'cinnamon-theme-void'
#        'cinnamon-theme-zukitwo'
#        'omg-cinnamon-theme'
)

build() {
  return 0
}
