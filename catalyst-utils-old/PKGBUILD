pkgname=catalyst-utils-old
pkgver=9.3
pkgrel=1
pkgdesc="Old proprietary AMD/ATI kernel drivers for Radeon brand cards. Support for R300/R700 chipsets. Stock kernel."
arch=('i686' 'x86_64')
url="http://www.ati.amd.com"
license=('custom') 
#finger required for acpi scripts
depends=("xorg-server<1.6.0" 'netkit-bsd-finger' \
...
build() {
  ...
  # bad fix for fglrx_dri.so (64-bit)
  if [ "${CARCH}" = "x86_64" ]; then
    mkdir -p ${pkgdir}/usr/lib64/dri/
    ln -sf /usr/lib/xorg/modules/dri/fglrx_dri.so ${pkgdir}/usr/lib64/dri/fglrx_dri.so
  fi
