...
# 2001-2002 Masaki Shinomiya <shino@pos.to>
echo "fax: version 0.9a001114b0 (11 Feb 2005)"
# 11 Feb 2005 bugfix handling '-m' option, accept periods(.) in fax number
#  4 Feb 2004
#  avoid errors in case remote fax machine's id contains spaces - Steven Doerfler <steven@lugaru.com>
#  bugfix in function faxmail adding boundary - Marc Ferraton <mferrato@ogm-techno.org>
# 27 Jan 2004
#   accept '_054' as ','(comma), thanks to Dennis Warner
# 29 Jan 2002
#   avoid 'bad units' error under some locale
# 6 Jan 2002
#   fax spooling via samba
#   accept escape sequence, '_043'as'#', '_052'as'*'
#   bugfix 'fax start/stop' command
# 15 Dec 2001 Masaki Shinomiya
#   faxlpr to work with LPRng
# 30 Sep 2001 by Masaki Shinomiya
#   allow to use INCOMINGDIR and OUTGOINGDIR separately
# 19 Aug 2001 by Masaki Shinomiya <shino@pos.to>
#   modified for the command 'fax start' to work and some misc
# 
...
ERR=0
[ "$INCOMINGDIR" ] || INCOMINGDIR=$FAXDIR
[ "$OUTGOINGDIR" ] || OUTGOINGDIR=$FAXDIR
...
faxmail () {
   ...
   do
     ...
     echo ""
     $EFIX -M <$f
# this line discard good attachement of next page if exist - Marc Ferraton <mferrato@ogm-techno.org>
#     echo "--EFAX_MAIL--"
   done
   echo "--EFAX_MAIL--"
}
...
# fax answer : clean up logs and exec efax. normally run by init(8).
	...
	if cd $INCOMINGDIR ; then :
	else
		echo "Error: $FAX cannot cd to $INCOMINGDIR" >>$CONSOLE
		sleep 30
		break
	fi
	...
	do
	  ...
	  1)    FILES=`sed -n -e  '/received ->/s/^.*-> \(.*\)$/\1/p' $f`
		export FILES=`echo $FILES`
		# 4 Feb 2004  REMID value  must be quoted to avoid
		# errors in case remote fax machine's id contains spaces
 		# informed by Steven Doerfler <steven@lugaru.com>
		export REMID="`sed -n -e '/remote ID ->/s/^.*-> \(.*\)$/\1/p' \
...
# fax queue : list received fax files
	...
	q*)
 	...
 	cd $INCOMINGDIR
 	case $? in 0) ;; *) echo "cannot cd to $INCOMINGDIR" ; break ;; esac
...
# faxlpr : get phone number and user from current cf* file and run fax send 
	...
	echo "$0: running as:" > $CONSOLE ; id -a >> $CONSOLE; env >> $CONSOLE
	${LS} -ld $OUTGOINGDIR >> $CONSOLE

	# the lpr spool directory for printer 'fax'
	if ! cd $OUTGOINGDIR
	then 
		echo "$0: cannot cd to $OUTGOINGDIR" >> $CONSOLE ; break 
	fi
...
if [ -n "$CONTROL" ]
  ...
  # originally by Luca Montecchiani (08/11/97 m.luca@usa.net)
  if [ -n "$CONTROL_FILE" ]
    then
        cfile=$CONTROL_FILE
    else
	if ! test -r lock
		then 
		echo "$0: can't read lock file" >> $CONSOLE  ; break 
	fi
	cfile=`tail -1 lock`
	if ! test -r $cfile
	then
		echo "$0: can't read control file" >> $CONSOLE  ; break 
	fi
	cfile=`cat $cfile`
  fi                                       
fi
	...
	if ! test "$num"
	then
		echo "$0: can't read phone number" >> $CONSOLE  ; break 
	fi

	# save stdin in a file
	if ! cat - >> fax$$
	then
		echo "$0: can't write `pwd`/fax$$" >> $CONSOLE  ; break 
	fi

	# convert  form file name: 30 Dec 2001 Masaki Shinomiya
        # originally by Msayo Ichiraku <daifuku@a1.mbn.or.jp>
        numorg=$num
	num=$(echo "$num" |  \
              sed  -e 's/^[A-Za-z][A-Za-z]*-//'  -e 's/\.[A-Za-z]*$//' )
...
#	 l=$numorg
	...
	*) echo -e "$numorg\n$l" | mail -s "fax to $num failed   " $user@$host
	  # move logs to $LOGDIR: 3 Dec 2001 Masaki Shinomiya
	  logfiles=$($LS *.log)
	  [ "$logfiles" != "" ] && (chmod 666 $logfiles;mv -f $logfiles $LOGDIR)
	;;
	esac
...
# fax start/stop/status : manage fax receive daemon
	...
	case $? in 0) ;; *) echo "cannot cd to $INCOMINGDIR" ; break ;; esac
        ...
        # 25 Dec 2001 by Masaki Shinomiya <shino@pos.to>
        # for the command 'fax start' to work
	n= ; for f in ${DEVN}.* ; do logfile="$f" ; n=x$n ; done
	case $n in 
	xx*) echo Warning: multiple logs for $DEV : ; ${LS} ${DEVN}.* ;;
	esac
...
# fax stop - make a stop file and kill current fax answer daemon
	...
	if [ ! -w . ] ; then echo "can't write `pwd`" ; break ; fi
        # 25 Dec 2001 by Masaki Shinomiya <shino@pos.to>
        #  touch ${DEVN}.stop only if killing process succeeded
	echo stopping fax daemon for ${DEV}, pid=$efaxpid
	kill -HUP $efaxpid && touch ${DEVN}.stop
	break 
	;;
...
# fax send : fax files to given number, converting first if necessary
	...
	'') 	;; 
	*)
	    # not pass when $TELNO is null, accept period(.) : 11 Feb 2005
	    # convert special characters: 27 Jan 2004 Masaki Shinomiya
	    TELNO=$(echo "$TELNO" |  \
              sed  -e 's/_043/#/g' -e 's/_052/*/g' -e 's/_054/,/g' \
                     -e 's/_43/#/g'   -e 's/_52/*/g' -e 's/_54/,/g' \
                     -e 's/%23/#/g'   -e 's/%2a/*/g' -e 's/%2c/,/g')
	    if ( [ "$TELNO" = "" ] || echo "$TELNO" | grep -q [^-#*0-9,.]) ; then
		echo "Illegal character(s) in fax number: $TELNO";ERR=2; break 2
	    fi
...
# fax (rm|print|view) commands possibly on files in spool directory
	...
	for f in $OUTGOINGDIR/$1 ; do 
		if test -r $f ; then cd $OUTGOINGDIR ; break ; fi
	done
...
cat 1>&2 <<EOF
 ...
 fax device is /dev/$DEV, incoming spool directory is $INCOMINGDIR
