...
# run
if [ -z "$GGB_PATH" ]; then
	exec java $JAVA_OPTS -jar /usr/share/java/geogebra5-beta/geogebra.jar --showSplash=false $GG_OPTS
else
	exec java $JAVA_OPTS -jar /usr/share/java/geogebra5-beta/geogebra.jar --showSplash=false $GG_OPTS "$GGB_PATH"
fi