pkgdesc="ROS - C++ parser for the Collada decription format." 
...
build() {
  ...
  if [ -d ${srcdir}/collada_parser ]; then
    ...
    git reset --hard release/groovy/collada_parser/${pkgver}-0
  else
    git clone -b release/groovy/collada_parser/${pkgver}-0 git://github.com/ros-gbp/robot_model-release.git ${srcdir}/collada_parser
  fi
