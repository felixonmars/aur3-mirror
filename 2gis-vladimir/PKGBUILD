pkgname=2gis-vladimir
pkgver=7
pkgrel=1
pkgdesc="Map of Vladimir for 2GIS, September 2012"
arch=('i686' 'x86_64')
url="http://vladimir.2gis.ru/how-get/linux/"
license=('custom')
depends=('2gis>=3.9.0.1')
source=("http://download.2gis.ru/arhives/2GISData_Vladimir-7.orig.zip")
md5sums=('bf50f3e9e7730e022ecbf7c3fdeb9eeb')

build() {
  cd $startdir
# Installing to /opt/2gis
  install -D -m 644 ${startdir}/src/2gis/3.0/Data_Vladimir.dgdat "${startdir}/pkg/opt/2gis/vladimir.dgdat" || return 1
  
}
