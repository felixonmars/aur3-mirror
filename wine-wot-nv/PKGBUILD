...
pkgdesc="Wine with patches for World Of Tanks. NVIDIA version"
...
depends=(
  ...
  nvidia-libgl    lib32-nvidia-libgl
  desktop-file-utils
)
...
#  msg2 "Applying patch for ATI..."
#  patch -p1 < "$startdir/0001_disable_arb_sync.patch" || exit 1
  # up fps
  ...
  if [[ $CARCH == x86_64 ]]; then
        ...
        ../$_pkgname/configure \
          ...
          --enable-win64 \
          --without-freetype
#	msg2 "Workaroud relpath build error ... "
#	mkdir -p "$srcdir/$_pkgname-64-build/tools"
#	cp "$srcdir/$_pkgname/tools/relpath.c" "$srcdir/$_pkgname-64-build/tools/"
#	cd tools
#	gcc -I../include relpath.c -o relpath
#	cd ..
        make
        _wine32opts=(
          --libdir=/usr/lib32
...
#  if [[ $CARCH == i686 ]]; then
#    msg2 "Workaroud relpath build error ... "
#    mkdir -p "$srcdir/$_pkgname-32-build/tools"
#    cp "$srcdir/$_pkgname/tools/relpath.c" "$srcdir/$_pkgname-32-build/tools/"
#    cd tools
#    gcc -I../include relpath.c -o relpath
#    cd ..
#  fi
