...
"  Description:     
"  Version:         1.5.1
"  LastChange:      2011-08-20 20:13:43
"  History:         Modified by ytj000@gmail.com
"=============================================================================
...
function s:AddTitle()
    ...
    let oldline = getline('.')
    call NERDComment(0, "minimal")
    let newline = getline('.')
    if oldline != newline
        let hasMul = 1
        let preChar = '#'
    else
        call NERDComment(0, "alignLeft")
        let newline = getline('.')
        if oldline == newline
            let hasMul = -1
            let noTypeChar = '#'
    ...
    if hasMul == 1
        exe firstLine.','.lastLine.'call NERDComment(0, "minimal")'
    else
        exe firstLine.','.lastLine.'call NERDComment(0, "alignLeft")'
    endif
