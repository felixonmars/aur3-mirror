...
package() {
    ...
    echo 'home=$(eval echo ~${SUDO_USER})'                      >> $_f
    echo 'dir="$home/.thedarkmod"'                              >> $_f
    echo 'if [ ! -d "$dir/fms" ]; then'                         >> $_f
    echo '    mkdir -p "$dir/fms"'                              >> $_f
    echo "    cp -R \"${_installdir}/fms/*\" \"\$dir/fms\""     >> $_f
    echo "fi"                                                   >> $_f
    echo './thedarkmod.x86 +seta fs_modSavePath \"$dir\" +seta fs_savepath \"$dir\"'     >> $_f
    ...
    # Actually no permissions have to be set with my custom executable, it sets the filepaths to whichever user's home folder.
