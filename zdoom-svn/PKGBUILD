...
pkgver=4337
pkgrel=1
pkgdesc="An enhanced Doom port with additional support for Heretic, Hexen and Strife (SVN version)."
arch=('i686' 'x86_64')
...
provides=('zdoom')
conflicts=('zdoom')
source=(zdoom::svn+http://mancubus.net/svn/hosted/zdoom/zdoom/trunk \
        http://www.fmod.org/index.php/release/version/fmodapi${_fmodver}${_fmodarch}.tar.gz \
...
pkgver() {
  svn info zdoom --xml | sed -rn '/<entry/{:0;s/^.*revision="//;t1;N;b0;:1;s/".*$//;p}'
}
