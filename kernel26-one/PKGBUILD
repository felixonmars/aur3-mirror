...
# Former Maintainer: Joao Rodrigues <gothicknight (at) gmail (dot) com>
# Original PKGBUILD: Andreas Radke <andyrtr (at) archlinux (dot) org>

pkgname=kernel26-one
_basekernel=2.6.39
...
package() {
  ...
  sed -i -e "s/KERNEL_VERSION=.*/KERNEL_VERSION=${_kernver}/g" $startdir/kernel26-one.install
