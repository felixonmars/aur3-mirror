...
pkgver=0
pkgrel=11
pkgdesc="GRASS GIS (stable 6.4.x SVN branch): geospatial data management and analysis, image \
...
source=("${pkgname}::svn+http://svn.osgeo.org/grass/grass/branches/releasebranch_6_4")
md5sums=('SKIP')
...
prepare() {
  ...
  # Custom desktop file:
  sed -i -e "s,^Name=GRASS GIS 6\$,Name=GRASS GIS 6.4.x SVN r${pkgver}," \
...
package() {
  ...
  ln -sf "/opt/${pkgname}/bin/gem64" "${pkgdir}/usr/bin/gem64-svn"
