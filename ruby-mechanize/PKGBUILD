# Generated by gem2arch (https://github.com/anatol/gem2arch)
# Maintainer: Anatol Pomozov <anatol.pomozov@gmail.com>

_gemname=mechanize
pkgname=ruby-$_gemname
pkgver=2.7.3
pkgrel=1
pkgdesc='The Mechanize library is used for automating interaction with websites'
arch=(any)
url='http://mechanize.rubyforge.org'
license=(MIT)
depends=(ruby ruby-net-http-digest_auth ruby-net-http-persistent ruby-mime-types ruby-http-cookie ruby-nokogiri ruby-ntlm-http ruby-webrobots ruby-domain_name)
options=(!emptydirs)
source=(https://rubygems.org/downloads/$_gemname-$pkgver.gem)
noextract=($_gemname-$pkgver.gem)
sha1sums=('74a8ac3c37477927771ca7a5624fb36311239724')

package() {
  local _gemdir="$(ruby -e'puts Gem.default_dir')"
  gem install --ignore-dependencies --no-user-install -i "$pkgdir/$_gemdir" -n "$pkgdir/usr/bin" $_gemname-$pkgver.gem
  rm "$pkgdir/$_gemdir/cache/$_gemname-$pkgver.gem"
  install -D -m644 "$pkgdir/$_gemdir/gems/$_gemname-$pkgver/LICENSE.rdoc" "$pkgdir/usr/share/licenses/$pkgname/LICENSE.rdoc"
}
