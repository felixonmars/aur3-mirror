...
pkgname=peazip-beta-qt 
pkgver=3.1
namefile=peazip-$pkgver.beta.LINUX.Qt
pkgrel=1 
pkgdesc="QT archiver utility (BETA VERSION)" 
arch=('i686') 
license=('LGPL') 
depends=('gcc-libs' 'gdk-pixbuf' 'p7zip' 'quad' 'balz' 'unace' 'upx' 'curl' 'gmp' 'gtk2' 'qt') 
conflicts=('peazip-gtk1' 'peazip-portable-gtk1' 'peazip-portable-gtk2' 'peazip-portable-qt' 'peazip-beta-gtk2') 
...
md5sums=('e6ea093abff6fddddc3ac7316c7af80c'
...
build() { 
  ...
  # Additional files 
  # Lib QT4
  install -m 644 ${srcdir}/usr/local/share/PeaZip/libqt4intf.so ${pkgdir}/usr/lib/${namedest}/libqt4intf.so
