...
license=('GPL')
depends=('postgresql' 'php-pgsql' 'php' 'net-snmp'  'curl' 'php-gd' 'fping')
...
md5sums=('575c31880d73f6fe41e730874ebfc633'
         ...
         '81d294474581c0a1cb7d0ceabec3d17d')

build() {
  cd $startdir/src/zabbix-$pkgver
  ./configure --prefix=/usr --enable-proxy --with-pgsql --with-net-snmp  --with-libcurl
  make || return 1
