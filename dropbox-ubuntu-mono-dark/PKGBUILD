...
pkgname=dropbox-ubuntu-mono-dark
pkgver=2.0
pkgrel=2
pkgdesc="Dropbox 0.8x icons for ubuntu-mono icon theme - dark version."
arch=('any')
...
makedepends=()
provides=()
conflicts=('dropbox-humanity-dark-icons' 'dropbox-dark-panel-icons' 'dropbox-humanity-icons' 'dropbox-light-panel-icons' 'dropbox-ubuntu-mono-light')
...
build() {
  cd $srcdir
  tar zxvf ./dropbox-ubuntu-mono-black.tar.gz || return 1
...
package() {
  ...
  mkdir -p $pkgdir/opt/dropbox/icons/hicolor/16x16/status
  cp * $pkgdir/opt/dropbox/icons/hicolor/16x16/status
}