...
pkgname=ppsspp-bin
pkgver=v0.9.8.r1526.gda32e5d
pkgrel=1
pkgdesc="A PSP emulator for Android, Windows, Mac, and Linux, written in C++. (binary version)"
arch=(i686 x86_64)
...
if [[ $CARCH = "i686" ]]; then
    _pkgver=$(curl -s "http://ppsspp.angelxwind.net/builds/Linux-Generic/i386/Karen_angelXwind/Version-stable.txt")
    source+=("http://ppsspp.angelxwind.net/builds/Linux-Generic/i386/karenbuildbot-net.angelxwind.ppsspp-linux-i386-generic.tar.gz")
else
    _pkgver=$(curl -s "http://ppsspp.angelxwind.net/builds/Linux-Generic/amd64/Karen_angelXwind/Version-stable.txt")
    source+=("http://ppsspp.angelxwind.net/builds/Linux-Generic/amd64/karenbuildbot-net.angelxwind.ppsspp-linux-amd64-generic.tar.gz")
fi
