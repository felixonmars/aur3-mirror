...
pkgname=wacom-linux-git
pkgver=20120107
pkgrel=1
pkgdesc="Kernel drivers for Wacom tablets, stable branch"
arch=('i686' 'x86_64')
...
conflicts=('linuxwacom-cvs' 'input-wacom' 'input-wacom-cvs' 'linuxwacom-baboo-cth-ctl'
           'wacom-next-linux-git' 'wacom-mainline-linux-git')
install=wacom-linux-git.install

_gitrepo="linux/kernel/git/stable"
_gitname="linux-stable.git"
...
function _get_source() {
  ...
  wget -q "http://git.kernel.org/?p=$_gitrepo/$_gitname;a=blob_plain;f=drivers/$_dir/$_file;hb=refs/heads/linux-$_ver.y" -O $_file
}
