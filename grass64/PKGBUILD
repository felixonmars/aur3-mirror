...
pkgver='6.4.4'
pkgrel='4'
pkgdesc="GRASS GIS (6.4.4 release): geospatial data management and analysis, image \
...
source=("http://grass.osgeo.org/grass64/source/grass-${pkgver}.tar.gz")
md5sums=('4b3e0caaeb1567e15c78b523e3674170')

prepare() {
  mv "${srcdir}/grass-6.4.4" "${srcdir}/${pkgname}"
  cd "$pkgname"
  ...
  # Custom desktop file:
  sed -i -e "s,^Name=GRASS GIS\$,Name=GRASS GIS 6.4.4 release," \
...
package() {
  ...
  ln -sf "/opt/${pkgname}/bin/gem64" "${pkgdir}/usr/bin/gem64"
