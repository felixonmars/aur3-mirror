pkgname=ppcrossmipsel
pkgver=2.6.0
pkgrel=1
pkgdesc="Free Pascal cross compiler for MIPS (little endian)"
arch=('i686' 'x86_64')
...
package() {
  ...
  cp "${srcdir}/fpcbuild-$pkgver/fpcsrc/compiler/ppcmipsel" ppcrossmipsel
  mkdir -p "$pkgdir/usr/bin"
  cd "$pkgdir/usr/bin"
  ln -s "/usr/lib/fpc/$pkgver/ppcrossmipsel" .
