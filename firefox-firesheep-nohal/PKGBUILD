...
pkgname='firefox-firesheep-nohal'
pkgver=1.5
pkgrel=1
_ffver=`pacman -Q firefox | cut -f2 -d\ | cut -f1 -d-`
pkgdesc="A Firefox extension that demonstrates HTTP session hijacking attacks (nohal version)"
arch=('i686' 'x86_64')
#url='http://codebutler.github.com/firesheep/'
url='https://github.com/obilodeau/firesheep'
license=('GPL')
depends=('firefox>=6.0' 'libpcap')
...
_gitroot=https://github.com/obilodeau/firesheep.git
...
build()
{
	...
	#fi
	#
	cat xpi/install.rdf|sed s/"maxVersion>7.0.\*"/maxVersion\>10.0.*/ > xpi/install.rdf.new
