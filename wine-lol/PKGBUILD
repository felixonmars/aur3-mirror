...
build() {
  ...
  (    
    cd $pkgname
    patch -Np1 -i "${srcdir}/0001-HACK-Disallow-block-based-textures-with-sub-block-si.patch"
    patch -Np1 -i "${srcdir}/0002-D3D-command-stream.patch"
  )
  ...
  make CFLAGS+="-mstackrealign -mincoming-stack-boundary=2" CXXFLAGS+="-mstackrealign -mincoming-stack-boundary=2" -j`grep processor /proc/cpuinfo | wc -l`
}
