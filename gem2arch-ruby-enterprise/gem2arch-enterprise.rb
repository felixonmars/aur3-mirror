...
PKGBUILD = %{# Generated by gem2arch-ruby-enterprise
...
url="<%= website %>"
license=('')
depends=('ruby-enterprise'<%= depends %>)
  ...
  depends = if depends.empty? then ""
    ...
    ' ' + depends.map do |d|
      d.requirement.requirements.map do |comp, ver|
        comp = '>=' if comp == '~>'
        "'ruby-enterprise-#{d.name}#{comp}#{ver}'"
      end
    end.join(" ")
  end
