...
source=("http://golang.org/dl/go$pkgver.src.tar.gz" 'go.sh')
md5sums=('2cdbad6baefcf1007f3cf54a5bc878b7'
...
build() {
  ...
  for platform in $_platforms ; do
    ...
    if [ $n -eq 1 ] ; then
      ...
      $srcdir/go/bin/go get -d code.google.com/p/go.tools/cmd/vet
      $srcdir/go/bin/go build -o $srcdir/go/pkg/tool/$platform/vet code.google.com/p/go.tools/cmd/vet
    fi
  done
}
...
package() {
  ...
  install -Dm644 LICENSE \
    $pkgdir/usr/share/licenses/go-crosstool/LICENSE
