...
provides=("crossover")
conflicts=("crossover" "crossover-standard")
arch=('i686' 'x86_64')
...
build() {
	cd "${srcdir}"
	ar -p crossover-pro-demo_${pkgver}-1_i386.deb data.tar.gz | tar zxf - -C "${pkgdir}" || return 1
	...
	mv "${pkgdir}"/usr/share/doc/crossover-pro-demo/* "${pkgdir}"/opt/cxoffice/doc
	gzip -d "${pkgdir}"/opt/cxoffice/doc/license.txt.gz
	install -m 644 -D "${pkgdir}"/opt/cxoffice/doc/license.txt "${pkgdir}"/usr/share/licenses/crossover-pro/license
...
md5sums=('a87a6182450bbc62d35b1cd8e3a72f2f'
