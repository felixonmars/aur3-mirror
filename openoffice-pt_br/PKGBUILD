...
replaces=('openoffice-base-bin' 'openoffice-base-bin_pt-br')
pkgver=4.0.1
pkgrel=1
pkgdesc="OpenOffice - i18n ptBR Brazilian Portuguese, repackaged from upstream"
arch=('i686' 'x86_64')
...
in i686 )
	...
	sha256sums[0]='6d4a1442744561478bb8c53c286bb4721085938f7c5cdcf0b76b0a4e746c0b74'
;; x86_64 )
	source[0]+="Apache_OpenOffice_${pkgver}_Linux_x86-64_install-rpm_${_lang}.tar.gz"
	sha256sums[0]='feb21e85881ef4b016a66bf1a8f7fa521343db1a2d5e2ce5074cda58afaa0e4a'
esac
...
package() {
	...
	sed -i -re "1s;^#! *(/usr(/local)?)?/bin/(env +)?python(2[^ ]*)?( |$);#!/usr/bin/env python2 ;" $(
		find ${_install_path}/program/python-core-2.7.5/lib -type f -name '*.py'
	)
#	# link mozilla plugin
