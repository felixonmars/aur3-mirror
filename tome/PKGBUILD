...
pkgver=235
pkgrel=1
pkgdesc="A dungeon crawler similar to Angband, based on the works of Tolkien.  2.X.X series."
arch=('i686' 'x86_64')
url="http://www.t-o-m-e.net"
license=('custom')
depends=('libx11' 'ncurses')
#source=(http://www.t-o-m-e.net/dl/src/$pkgname-$pkgver-src.tar.bz2 \
source=(http://www.mirrorservice.org/sites/www.ibiblio.org/gentoo/distfiles/$pkgname-$pkgver-src.tar.bz2 \
        LICENSE)
...
build() {
  ...
  install -D -m755 "${srcdir}/$pkgname-$pkgver-src/src/$pkgname" "${pkgdir}/usr/bin/$pkgname"
  cp -R "${srcdir}/$pkgname-$pkgver-src/lib" "${pkgdir}/usr/lib/$pkgname/"
  ...
  find ${pkgdir}/usr/lib/tome -type f -name "delete.me" -exec rm "{}" \;
  find ${pkgdir}/usr/lib/tome -type f -name ".cvsignore" -exec rm "{}" \;
