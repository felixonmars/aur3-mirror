...
## Forked by GI_Jack <iamjacksemail@hackermail.com>
# This is GI_Jack's patched version that allows "." to be used in usernames with useradd.
# chckname.c is modified by GI_Jack, and licensed under the same terms as the rest of the package.

pkgname=shadow-jack
_pkgname=shadow
pkgver=4.1.5.1
pkgrel=7
pkgdesc="Password and account management tool suite with support for shadow files and PAM(patched to allow dotnames)"
arch=('i686' 'x86_64')
...
source=("http://pkg-shadow.alioth.debian.org/releases/$_pkgname-$pkgver.tar.bz2"{,.sig}
        LICENSE
        chgpasswd
        chpasswd
        defaults.pam
...
sha1sums=('81f38720b953ef9c2c100c43d02dfe19cafd6c30'
          ...
          '170eadb4c36add255ad7c87f436d4f1edb83633e')

build() {
  #use provided version of chckname.c which allows for usernames with "." dots in them.
  rm "${srcdir}/$_pkgname-$pkgver/libmisc/chkname.c"
  cp "${srcdir}/chkname.c" "${srcdir}/$_pkgname-$pkgver/libmisc/chkname.c"
