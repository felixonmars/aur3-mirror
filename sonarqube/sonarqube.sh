...
# since we have a depencency to java-runtime, one of these files must
# exist :-)
...
if [ -f /etc/profile.d/openjdk6.sh ]; then
...
export SONAR_RUN_DIR=${SONAR_RUN_DIR:=/opt/sonarqube/run}
export SONAR_ETC_DIR=${SONAR_ETC_DIR:=/opt/sonarqube/conf}
export SONAR_LOG_DIR=${SONAR_LOG_DIR:=/var/log/sonarqube/}
...
# Wrapper
WRAPPER_CMD="$SONAR_HOME/bin/wrapper"
WRAPPER_CONF="$SONAR_ETC_DIR/wrapper.conf"
