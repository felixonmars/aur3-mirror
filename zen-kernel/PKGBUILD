...
# AUR workaround...
pkgname="zen-kernel-stable-latest"
...
if [ "$_use_snapshot" = "y" ] && [ ! -d "$_sourcedir" ]; then
	source+=(http://gitorious.org/zen-kernel/zen-stable/archive-tarball/master)
	md5sums+=("20b0b1dbca9ad4c2c5844d94e4678b4f")
fi
...
fetch_sources() {
	if [ ! -d "$_gitname" ]; then
		...
		if [ "$_use_snapshot" = "y" ]; then
			...
			mv "$srcdir/zen-kernel-zen-stable" "$_sourcedir"
