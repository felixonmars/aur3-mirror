...
#
# Modified Japanese zip code data by Ibs
...
_svnmod=src
_depott_url="https://chromium.googlesource.com/chromium/tools/depot_tools.git"
...
makedepends=('pkg-config' 'python2' 'subversion' 'git')
changelog=ChangeLog
source=(
  "${pkgbase}/${_svnmod}::svn+${_svntrunk}"
  "git+${_depott_url}"
...
if [[ "$_zipcode" == "yes" ]]; then
  source+=('http://downloads.sourceforge.net/project/pnsft-aur/mozc/x-ken-all.zip'
           'http://downloads.sourceforge.net/project/pnsft-aur/mozc/jigyosyo.zip')
  sha1sums+=('SKIP' 'SKIP')
fi
...
prepare() {
  ...
  git config remote.origin.url $_depott_url
