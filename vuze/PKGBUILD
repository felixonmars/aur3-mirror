...
_ver=5000 # But this way people can download the file from the AUR page directly.
...
package() {
	...
	install -pm644 "$srcdir/Vuze_${_ver}${_extra}.jar" \
		"$pkgdir/usr/share/vuze/Azureus2.jar"

	# When the updater fails to install in system directories, the program
	# falls in an endless loop. And won't push to the user directories 
	# either. They released these loose updates less than 24 hours after 
	# the main release yet won't release a fixed linux package.
	# And what about all the other out-dated junk? Disappointing.

	install -pm644 "$srcdir/azrating_1.4.2.jar" \
		-t "$pkgdir/usr/share/vuze/plugins/azrating"
	unzip -qq -o -d "$pkgdir/usr/share/vuze/plugins/azupnpav" \
		"$srcdir/azupnpav_0.4.7.zip"
