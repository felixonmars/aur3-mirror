...
_unreal_handle_files() {
    ...
    while read stdin; do
        /usr/bin/find -- "$2" -regextype posix-egrep \
...
_unreal_move_files() {
    _unreal_handle_files "/bin/mv --force --" "$@" || return 1
}
...
_unreal_decompress_files() {
    while read stdin; do
        /usr/bin/find -- "$1" -regextype posix-egrep \
...
_unreal_loki_patcher() {
    local dest
    cd "$2"
    /bin/mkdir -p xdelta.tmp

    /usr/bin/find . -regextype posix-egrep \
    ...
    do
        case "$(file -b -- "${file}")" in
            (*XDelta*)
                ...
                ;;
            (*gzip*)
                /bin/gzip --decompress --stdout -- "${file}" > "$3/${file}" \
                        || "$1" || return 1
                ;;
            (*executable*)
                /bin/install --mode=755 -D -- "${file}" "$3/${file}" \
                        || "$1" || return 1
                ;;
            (*)
                /bin/install --mode=644 -D -- "${file}" "$3/${file}" \
                        || "$1" || return 1
                ;;
        esac
    done
