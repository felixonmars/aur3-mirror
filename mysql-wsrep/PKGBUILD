...
options=('!libtool')
source=(http://ftp.gwdg.de/pub/misc/mysql/Downloads/MySQL-5.6/${_pkgname}-${pkgver}.tar.gz{,.asc}
        https://launchpad.net/codership-mysql/5.6/${pkgver}-${_pkgrel}/+download/mysql-${pkgver}_wsrep_${_pkgrel}.patch
        mysqld-post.sh
        mysqld-tmpfile.conf
        mysqld.service
...
sha256sums=('70fe55985ba187a26cce6905c57f66bb9904b33760e3ff30b611697b4bf2fde0'
            'SKIP'
            '876c4b6bbb15c8cce14ed88d6e4669ecb7c0f08f581843a0465ea52d7a6c66d1'
...
build() {
  ...
  patch -p1 -N -f -i "${srcdir}"/mysql-${pkgver}_wsrep_${_pkgrel}.patch || true
  patch -p0 -N -i "${srcdir}"/mysql-srv_buf_size.patch || true
