...
pkgname=jack2-no-dbus-firewire-git
pkgver=1.9.9.5.245.g0ca1fb5
pkgrel=1
pkgdesc='Classic jack2 with firewire but without dbus enabled. For starting as daemon with systemd.'
arch=('x86_64' 'i686')
url='http://jackaudio.org/'
license=('GPL2')
depends=('libsamplerate' 'libffado')
...
build() {
  ...
  ./waf configure --classic --alsa --firewire --clients=128 --ports-per-application=1536 --prefix=/usr
  ./waf $MAKEFLAGS build 
}
