pkgname=mutt-xtermtitle
pkgver=1.5.20
pkgrel=4
pkgdesc="A small but very powerful text-based mail client (patched for displaying custom title in xterm)"
arch=(i686 x86_64)
license=('GPL')
...
build() {
  ...
  # patch xterm title
  patch -p1 < ${srcdir}/mutt-xterm_title.patch || return 1
  autoreconf
