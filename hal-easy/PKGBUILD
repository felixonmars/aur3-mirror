# Contributor: Edigarev Evgeniy <edigarev@gmail.com>

pkgname=hal-easy
pkgver=0.5.14
pkgrel=1
pkgdesc="Hardware Abstraction Layer without ConsoleKit and PolicyKit support"
arch=(i686 x86_64)
license=('GPL' 'custom')
url="http://www.freedesktop.org/wiki/Software/hal"
depends=('dbus-glib>=0.82' 'libusb>=0.1.12' 'udev>=146' 'filesystem>=0.7.1-5' 'hal-info>=0.20090716' 'eject' 'dmidecode' 'pciutils>=3.0.2' 'usbutils>=0.73-5' 'pm-utils>=1.2.5' 'util-linux-ng>=2.16')
makedepends=('pkgconfig' 'gperf')
provides=('hal=0.5.14')
conflicts=('hal')
options=('!libtool' '!makeflags')
install=hal.install
source=(http://hal.freedesktop.org/releases/hal-${pkgver}.tar.gz
        hal
...
build() {
  ...
  install -m755 -d "${pkgdir}/usr/share/licenses/hal"
  install -m644 COPYING "${pkgdir}/usr/share/licenses/hal/" || return 1
}
