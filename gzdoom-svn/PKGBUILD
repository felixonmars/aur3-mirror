...
pkgver=1594
pkgrel=1
pkgdesc="Doom source port based on ZDoom with an OpenGL renderer (SVN version)."
arch=('i686' 'x86_64')
...
provides=('gzdoom')
conflicts=('gzdoom')
source=(gzdoom::svn+http://mancubus.net/svn/hosted/gzdoom/trunk/ \
...
pkgver() {
  svn info gzdoom --xml | sed -rn '/<entry/{:0;s/^.*revision="//;t1;N;b0;:1;s/".*$//;p}'
}
