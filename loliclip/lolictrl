...
urlmode() {
   ...
   local url="$("$LOLICLIP" -l | grep -aoP "((($protos):\/\/|www\..*\.)[-/.\?=&%()#@~:\w]+)" | $DMENU -p "loliurl")"
   [[ -n "$url" ]] || exit 1
   echo "#LC:skip_history:$url" | "$LOLICLIP" -i --${CLIPBOARD}
   echo "$url"
   exit 0
}
...
# clipmode
clipmode() {
   id="$("$LOLICLIP" -m | $DMENU -p "loliclip" | sed 's/^[^0-9]*\([^:]*\):.*/\1/g')"
   [[ -n "$id" ]] || exit 1
   echo "$id" | "$LOLICLIP" -g | "$LOLICLIP" -i --${CLIPBOARD}
   sleep 0.25
   "$LOLICLIP" --${CLIPBOARD}
   exit 0
}
