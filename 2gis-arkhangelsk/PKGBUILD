pkgname=2gis-arkhangelsk
pkgver=35
pkgrel=1
pkgdesc="Map of Arkhangelsk for 2GIS, July 2014"
arch=('i686' 'x86_64')
url="http://info.2gis.ru/arkhangelsk/products/download#linux"
license=('custom')
depends=('2gis>=3.14.7.0')
source=("http://download.2gis.com/arhives/2GISData_Arkhangelsk-35.orig.zip")
md5sums=('945e7f23bfed17916ded9d66cc82bd4d')

package() {
  install -D -m 644 "${srcdir}/2gis/3.0/Data_Arkhangelsk.dgdat" "${pkgdir}/opt/2gis/2gis-arkhangelsk.dgdat" || return 1
  
}
