...
build() {
  ...
  make -f "$_svnmod/uppbox/lpbuild2/mkfile" PKG=ide FLAGS="GCC GUI NOGTK STACKTRACE" NESTS="$_svnmod/uppsrc" TIME= COLOR=0 USEMAINCFG=0 JOBS=5 ECHO=0 TARGET="$srcdir/theide"
  mv "$_svnmod/uppsrc/ide/version.h~" "$_svnmod/uppsrc/ide/version.h"
}
