...
pkgver=3.9_rc4_2_su # NO CRAP! (-c). Use _ instead of -
_upver="${pkgver//_/-}"
pkgrel=3
...
optdepends=('compat-drivers-frag+ack: wl-frag+ack patch')
...
source=("http://www.kernel.org/pub/linux/kernel/projects/backports/stable/v3.9-rc4/compat-drivers-${_upver}.tar.xz")
#echo $source
#XZ is way better in decompression

sha256sums=('718c1d6baaf82bea5383ada4c1cc4b772bd992f55d462d8339c7f6cbfb78cf27')
...
prepare() {
  (
    _kernel_simple=${_runkernver%.*}
    if [ -n "${_kernel_simple:2:2}" ]; then
      if [ ${_kernel_simple:2:2} -gt 9 ]; then
        error "compat-drivers-patched package remains in AUR only for old kernels. Use backports-patched instead"
        false
        exit 1
      fi
    fi
    unset _kernel_simple
  )
