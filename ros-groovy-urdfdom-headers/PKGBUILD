...
build() {
  ...
  if [ -d ${srcdir}/urdfdom_headers ]; then
    ...
    git reset --hard release/groovy/urdfdom_headers/${pkgver}-2
  else
    git clone -b release/groovy/urdfdom_headers/${pkgver}-2 git://github.com/ros-gbp/urdfdom_headers-release.git ${srcdir}/urdfdom_headers
  fi
