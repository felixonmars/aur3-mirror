# Maintainer: Eugene Lamskoy <e.lamskoy (at) gmail.com>
# Replacement for grub-gfx package
_real=grub
pkgname=grub-ext
pkgver=0.97
pkgrel=20
pkgdesc="A GNU multiboot boot loader with graphics support (based on original archlinux grub PKGBUILD)"
arch=('i686' 'x86_64')
license=('GPL')
...
build() {
  cd $srcdir/${_real}-$pkgver

  # optimizations break the build -- disable them
  # adding special devices to grub, patches are from fedora
  patch -Np1 -i ../special-devices.patch || exit 1
  patch -Np1 -i ../i2o.patch || exit 1
  patch -Np1 -i ../more-raid.patch || exit 1
  patch -Np1 -i ../intelmac.patch || exit 1
  # Add support for bigger inode size to e2fs_stage1_5
  patch -Np1 -i ../grub-inode-size.patch || exit 1
  # Add ext4 support
  # http://www.mail-archive.com/bug-grub@gnu.org/msg11458.html
  patch -Np1 -i ../ext4.patch || exit 1
  # binutils fix
  patch -Np1 -i ../grub-0.97-ldflags-objcopy-remove-build-id.patch || exit 1
  patch -Np1 -i ../grub-0.97-graphics.patch || exit 1

  sed -e'/^AC_PROG_CC/ a\AM_PROG_CC_C_O\ ' -i "${srcdir}/${_real}-${pkgver}/configure.ac"
  sed -e'/^AC_PROG_CC/ a\AM_PROG_AS\ ' -i "${srcdir}/${_real}-${pkgver}/configure.ac"
...
package() {
  ...
  install -D -m644 ../splash.xpm.gz $pkgdir/boot/grub/splash.xpm.gz
