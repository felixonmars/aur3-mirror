...
build() {
  ...
  if [ "$_rebase" -eq 1 ]; then
      ...
      git fetch upstream
      local ret=0
      # If user has never configured git on this system they wont have a
      # username and git will, for whatever reason, refuse to rebase. Set a
      # throwaway name (local to just this build repo)
      git config user.email "wine-multimedia-git@rebasing.patches.localhost"
      git config user.name "wine-multimedia-git auto-rebase"
