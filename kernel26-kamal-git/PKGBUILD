...
pkgdesc="The Linux Kernel 2.6.38 (ubuntu natty kernel version) with patchs to fix non-functional brightness control keys \
on some laptops (i.e Dell XPS, Dell Studio...) using i915 driver, see more: \
...
kernel_packages='http://kernel.ubuntu.com/git?p=kamal/ubuntu-natty.git;a=snapshot;h=refs/heads/linux-kamal-mjgbacklight;sf=tgz'
...
build() {
	...
	wget $kernel_packages -O linux-kamal-mjgbacklight.tar.gz
	tar zxvf linux-kamal-mjgbacklight.tar.gz
	_kernel_src="$_build_dir/$(find . -maxdepth 1 -type d -name "ubuntu-natty-*" -printf "%f\n" | head -1)"
