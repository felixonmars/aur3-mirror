...
build() {
  ...
  if [ -d ${srcdir}/image_geometry ]; then
    ...
    git reset --hard release/groovy/image_geometry/${pkgver}-0
  else
    git clone -b release/groovy/image_geometry/${pkgver}-0 git://github.com/ros-gbp/vision_opencv-release.git ${srcdir}/image_geometry
  fi
