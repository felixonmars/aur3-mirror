...
cp "$_install/theme_index/plank-themer.dockitem" "$_plank1/launchers/"

# Place dockitem in Plank (at start?)
const='plank-themer.dockitem'
if [ -e "$_plank1/settings" ]; then
	...
	if [ $? = 1 ]; then
		...
		sed -i 's@DockItems=plank.dockitem;;@DockItems=plank.dockitem;;plank-themer.dockitem;;@' "$_plank1/settings"
		sed -i 's@DockItems=plank.dockitem;;@DockItems=plank.dockitem;;plank-themer.dockitem;;@' "$_plank1/settings" 
		sed -i 's@plank-themer.dockitem\n@\n@g' "$_plank1/settings"
	fi
fi
