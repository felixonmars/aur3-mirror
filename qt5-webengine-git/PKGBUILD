# Maintainer: Jocelyn Turcotte <turcotte.j at gmail.com>
# Forked from https://aur.archlinux.org/packages/qtwebengine maintained by Stefan Damm <autama at nurfuerspam.de>

pkgname=qt5-webengine-git
pkgver=8fba2a4
pkgrel=1
...
conflicts=('qtwebengine')
source=("$pkgname"::'git://gitorious.org/qt-labs/qtwebengine.git'
        '3rdparty'::'git://gitorious.org/qt-labs/chromium.git')
md5sums=('SKIP'
         'SKIP')
...
prepare() {
  ...
  git submodule init
  git config submodule.3rdparty.url ${srcdir}/3rdparty
  ...
  find src/3rdparty/chromium/third_party -type f \! -iname '*.gyp*' \
    ...
    \! -path 'src/3rdparty/chromium/third_party/jinja2/*' \
    ...
    \! -path 'src/3rdparty/chromium/third_party/markupsafe/*' \
