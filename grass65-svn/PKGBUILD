...
pkgver=0
pkgrel=6
pkgdesc="GRASS GIS (testing 6.5.x SVN branch): geospatial data management and analysis, image \
...
source=("${pkgname}::svn+http://svn.osgeo.org/grass/grass/branches/develbranch_6")
md5sums=('SKIP')
...
prepare() {
  ...
  # Custom desktop file:
  sed -i -e "s,^Name=GRASS GIS 6\$,Name=GRASS GIS 6.5.x SVN r${pkgver}," \
         -e "s,^Icon=grass\$,Icon=/usr/share/icons/${pkgname}-64x64.png," \
         -e "s,^Exec=grass65\$,Exec=${pkgname}," \
...
package() {
  ...
  sed -i "s,${pkgdir},,g" "${pkgdir}/opt/${pkgname}/include/Make/Platform.make" \
                          ...
                          "${pkgdir}/opt/${pkgname}/bin/grass65"

  # Link GRASS exec script and gem binary in /usr/bin under a custom name.
  # This allows e.g. grass65 and grass65-svn be co-installed:
  mkdir -p "${pkgdir}/usr/bin"
  ln -sf "/opt/${pkgname}/bin/grass65" "${pkgdir}/usr/bin/${pkgname}"
  ln -sf "/opt/${pkgname}/bin/gem65" "${pkgdir}/usr/bin/gem65-svn"
