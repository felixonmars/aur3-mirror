...
md5sums=('101aab0ef09510fd12b7d5718d06dd28')
...
# the build function will be common for all gnu2heirloom packages of a given type
  ...
  msg "copy scripts and symlinks for missing utilities corresponding those in coreutils"
...
# Moving manpages into a common directory
   ...
   for i in $(cat $_ucb)
      ...
      mv $srcdir/tmp/usr/heirloom/share/man/man1/true.1 $srcdir/tmp/man/true.1
      mv $srcdir/tmp/usr/heirloom/share/man/man1/false.1 $srcdir/tmp/man/false.1
  ...
  for i in $(cat $_busybox)
      do
      printf "Busybox symlink. For more info type busybox $i --help" > $i.1
    done
